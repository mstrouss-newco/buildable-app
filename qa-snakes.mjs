...
// snakes-engine.html loads buildable-wincard.js in the browser; the sandbox needs it
// too, or the win render throws 'BuildableWin is not defined' (same bug as RN5/maze).
...
const noop=()=>{};
const ctxStub=new Proxy({},{get:(_,k)=>(k==='createLinearGradient'||k==='createRadialGradient')?()=>({addColorStop:noop}):(k==='canvas'?{width:720,height:1040}:(k==='measureText'?((t)=>({width:String(t||'').length*8})):(typeof k==='string'?noop:undefined)))});
