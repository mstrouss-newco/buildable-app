...
- **One consistent HUD + nav rule, rolled out to every game.** The app shell owns the top strip: Home top-left, Sound/Menu/Help top-right. Games route their own buttons through the shared `buildable-gamenav.js` bridge (hides them in-app, shell drives them) instead of drawing competing buttons. Added the bridge to the five games that were still doing their own thing: runner, survival, tank, bubble, mahjong (board games, breaker, tetris, castle, croc already had it). The shared HUD (`buildable-hud.js`) already insets to clear the nav.
