...
## The studio puts the child's own idea on the screen (September 15 2026)
**Phase CB, cards CB6 and CB7, run as one session.** The lander promises that a dragon who delivers pizza to the moon becomes a game about THAT; the 2026-09-06 QA run found it became stock Sky Flyer with a new title, and three things caused it. FIRST, THE PLAN ASKED FOR ART BY THEME ONLY, so the child's sentence never reached the picture. The plan now carries a SHOT LIST: one entry per picture the game needs, each naming a slot that engine's own `cobuild.json` really has (so a picture is never painted with nowhere to hang), a short painting description built out of the child's words, a search phrase the shared library is checked with first, and the noun from the sentence it is carrying. Six pictures is the aim and ten the ceiling. THE NOUNS COME FROM WHAT FOLLOWS "A", "AN" OR "THE", because a child names the thing they mean with an article in front of it almost every time: "a robot cat who races trains under the sea" gives "robot cat" and "sea", not "races". Each engine gets the shots it has slots for and no others, which is why Breaker gets a hero on the bat, a world behind every level and a brick set, Castle Guard gets a hero, the goblin at the end and a badge a level, and Sky Flyer, whose worlds are a palette NAME rather than a picture, gets a hero and a badge a level rather than a painting dropped into a slot that would blank the world. SECOND, REUSE MEANT "ANOTHER PICTURE OF THE RIGHT SHAPE": the kids lane handed back the first cobuild asset of the right kind and theme, so every space game got the first space picture ever painted. It now matches the shot's search words against the asset before reusing it, and files what it paints with that phrase in the slug AND the descriptor, so the second family who asks for a dragon gets one instantly and for nothing. Same daily cost brake, same `usage_log` line. THE PRE-KEEP CHECK is new and it is the safety net: before a game is written, `op:"artCheck"` asks whether the manifest really ended up wearing the child's idea — a painted hero, and a world on every level — and anything missing is repainted ONCE. It never blocks a child; a game with a gap is kept anyway and the honest line is stored on the row in `robot.art` instead of being hidden. `op:"art"` had to grow to make any of this land: it could only reach `levels[].parts.*` and it put one picture on every level, so it now hangs whole-game slots like `art.hero`, per-level badges, and a piece that names ONE level, which is how level two's world stops being level one's. Every version is still strict-validated against the sheet and anything that does not fit is dropped, not forced. THIRD, THE ENGINES DID NOT AGREE ABOUT WHOSE GAME IT WAS. `buildable-manifest.js` now owns three answers every engine used to invent for itself: the start-screen title, the browser tab, and a SAVE FILE KEYED BY THE KID GAME ID, so a brand new game never opens showing stock cleared levels or somebody else's stars. Sling Squad was the known offender and Breaker, Castle Guard and Sky Flyer had the same hole. ON THE WORDS: the star chips are built from the child's own nouns first and our stock heroes second, so a kid who only ever taps a chip still ends up with their own idea; and "That is not quite a game I can build yet" is gone along with every line like it — an idea no engine matches head-on is answered by NAMING the game being built with the child's star in the middle of it, and the tweak door, the voice recorder and the microphone fallback were reworded the same way. ON THE NAMES: the studio asks the child's first name and a grown-up's name once, keeps them on the device, and carries both on every save, so the cover, the Keep card, the share sheet and the `/g/` link preview title (which named only the kid) all say the same thing; and an edit can no longer wipe them, because a tweak that does not carry the names leaves the ones on the row alone, which is how a game used to quietly lose the family that made it. THE STUDIO FITS ONE SCREEN NOW: the play frame keeps its shape but never takes more than two thirds of the height, so the tweak chips are visible without scrolling at 1440x900 and at 390x844. Planner hygiene (CB7 item 6) was already done before this session: CB2, CB3 and CB4 were flagged deployed with corrected notes. `qa-studio.mjs` grew a CB6 section and a CB7 section, `qa-kidgames.mjs` now expects the share preview to name both makers, and `node qa-all.mjs` is green. STILL NEEDS MIKE: the family test with Jackson and Riley on a phone before CB8, and the card's acceptance check on the live site — build from "a robot cat who races trains under the sea" and look for a cat.

## The gate told the truth again, and the emoji came out of Typing (September 15 2026)
**Phase QA, card QA57 — three reds in `node qa-all.mjs`, found while running the gate for FM11 and each one checked against a pristine HEAD before it was touched.** FIRST AND WORST, EMOJI WERE LIVE IN FRONT OF KIDS. The typing rebuild put TWENTY-ONE emoji glyphs into `public/typing.html`: a lightning bolt, a dart board and a star in the header pills, a castle, a rocket and a stopwatch on the tile pictures, a loudspeaker on Read aloud, a tick on every finished lesson, a padlock on every locked one, and `★★★☆☆☆` sliced with `substr` for the star ratings. Every one is now DRAWN SVG GEOMETRY, held in an `ICO` table the markup and the JavaScript both read from, so a pill repainted by code is the same picture as the one that was there on first paint. The rule is worth restating because it is not fussiness: an emoji is A DIFFERENT PICTURE ON EVERY DEVICE a kid opens it on, and half of them are not the picture we meant. SECOND, FIVE OF THE SEVEN FAILURES WERE NOT THE PRODUCT AT ALL — the harness was pointed at a file that had moved. Typing is TWO pages since the rebuild: `typing.html` is the school and `typing-game.html` is Defend the Castle, which used to BE typing.html; the harness still grepped one file called typing.html for a fort, a hero face and a win message, found a page that had never had any of those in it, and went red five times over. The checks were right and they were aimed at the wrong file, so they now name the file they mean and the emoji scan runs over BOTH pages. THE EMOJI RANGE ALSO HAD A HOLE IN IT, which is the worse of those two bugs: the stopwatch is U+23F1 and the range list started at U+2600, so the file shipped two stopwatches while the harness called it clean apart from the two stray variation selectors hanging off the end of them — A RANGE THAT CATCHES THE ACCENT BUT NOT THE CHARACTER READS AS A PASS, which is worse than no check at all. THIRD, A SWITCH THAT SWITCHED NOTHING: `cobuild_live` in `/api/app-flags` has always said in its own note that when it is true the Start buttons on `/cobuild` go to real signup and Stripe checkout instead of the waitlist, and the page never read it, so flipping it would have changed nothing on the page it names. It reads it now, FALSE is still the default and still what is live, nothing a visitor sees changes today and nobody is charged; what is new is that the live path exists AND FALLS BACK RATHER THAN DEAD-ENDS, because a Start button that does nothing is the worst thing a marketing page can do, so if the signup is not ready (it is not — there is still no sign-up page, QA50), or the flag read fails, or the network is out, the family lands in the same friendly waitlist. FOURTH, THE EGG THE FARM WAS COLLECTING PERFECTLY ALL ALONG: `walking over the egg hops it onto the stack like any crop` has been red on and off since FM6 and the farm was never at fault, twice over now. Three faults in the harness, stacked. Its "stand clear of the magnet" step only stood clear of ONE hen's egg — it offset from the target hen by x+6.5, z+6.5, which is outside THAT hen's magnet, but `advanceTime` ripens every hen she fed and the coop's hens sit a couple of units apart, so the diagonal routinely landed inside four units of a DIFFERENT hen's egg and those eggs flew onto the stack before the reading was taken. It counted THINGS rather than EGGS, and she is usually still carrying corn, and collecting an egg while holding corn feeds that hen straight back up — the loop closing, which the cow checks two blocks down call out by name — so the stack height moves for reasons that have nothing to do with this egg. And it waited on the wrong word: it waited for the hen to stop being `ready`, but a hen fed again on the spot goes `ready` to `feeding` to `making`, so a harness watching for "not ready" can read the word it wanted WHILE THE EGG IS STILL SITTING IN THE GRASS. It now finds a spot clear of every produce spot on the farm and proves it is clear, counts eggs, and waits for that hen's egg to be gone from the ground. Traced by replaying the harness step by step in a real browser and printing the state every second: three hens ready, and the instant she was put on the target egg the stack read `[egg, egg]` — hers and the neighbour's, 1.7 units apart, both inside the magnet. LAST, THE GATE CAN BE RUN HONESTLY NOW, because a gate that invents red is worse than no gate: the next person learns to ignore it. The hard-coded five-minute kill became a per-harness cap (25 minutes for the seven that really do play a whole game through in a real browser, 5 for the rest) with a `--timeout <minutes>` override, and a killed harness SAYS IT RAN OUT OF TIME instead of surfacing as `exit null` beside whatever half-finished output had scrolled by. `qa-skyflyer-hud`, `-look` and `-sky` each expected somebody to have run `python3 -m http.server 8899` in another window and died with `ERR_CONNECTION_REFUSED` when nobody had; new `scripts/qa-serve.mjs` hands any harness a base URL — an existing server if something is answering, otherwise `public/` on a free port — and all three run from a cold start. Those same three pinned `/opt/pw-browsers/chromium-1194/...`, which goes stale the day the image is rebuilt, and required `playwright-core`, which is not a repo dep so `npm ci` deletes it every time; `qa-ap2-use-in-game.mjs` and `qa-kp3-add-a-kit.mjs` went further and imported playwright by an ABSOLUTE PATH INTO ONE PARTICULAR LAPTOP'S npm FOLDER, so on any other machine they died on the import line before printing a word and `qa-all.mjs` read that as a broken product. All five now find chromium on disk whatever it is called, look for playwright in the places it actually lives, and print one plain line instead of a module-resolution stack. AND THEN FM12 LANDED ON MAIN WHILE THIS WAS IN FLIGHT, RED: merging main in brought twelve fresh failures that had already been pushed and deployed, nine in `qa-skyflyer.mjs` and three in `qa-farm.mjs`, all confirmed on a pristine `origin/main` worktree before anything was touched, and every one of them the harness being stale rather than the game being broken — the same shape as the bugs above, a check that names a number somebody has to remember to retype. THE BUILD TAG WAS TYPED OUT IN EIGHT PLACES (`version: "fm8"` twice in the static half and five times in the farm robot, plus `?v=fm10` four times over) and FM12 bumped the engine and the door and left all eight behind; there is now ONE source of truth, the cache-bust on the door in `BuildableKids.jsx` which is what a kid's browser actually fetches, and every check reads the tag from there and asks the engine to agree with it, so a card that bumps the build no longer has to find eight places and a build where the door and the engine DISAGREE now goes red with both numbers in the message, which is a genuinely useful check and is not one we had. THE BRIDGE BECAME A DOCK on Mike's word — he looked at the live farm and said a pond that size wants a dock you can stand on the end of, not a full bridge — so the check that grepped for `buildBridge` by name went red on the thing it asked for, and it asks for A WAY ONTO THE WATER now. THE ARROW'S HOP WAS PINNED TO THE PIXEL at `baseY+u*0.38`, so FM12 could not calm it to `0.30` without turning FM10 red: up is the rule and the height is a dial. Two more flaky checks were fixed at the premise rather than the assertion: `an egg left on the ground` demanded a chicken that happened to be HUNGRY at that moment and returned a bare false when there wasn't one, a coin toss decided by whatever the blocks above left behind; and `the animal she is nearest to STOPS pacing` waited for a patrolling hen to go loud when FM10's own rule is that only a HUNGRY animal gets loud, so it was waiting twenty-five seconds for something that could not happen. Both now carry a reason in their message, because a red that says nothing teaches people to ignore reds.

## The Farm: the stick comes to her thumb (September 15 2026)
**Phase FM, card FM11 — Mike's playtest: "you have to use the button controller now and it stinks".** He is right, and the code agreed with him: the only way to walk was a 130px pad nailed to the bottom-left corner. THE STICK NOW GOES TO HER. Put a thumb down anywhere on the farm and slide, and that spot becomes the middle of the stick: it fades in under her thumb and fades out when she lifts, left hand or right, top of the screen or bottom, so her thumb never hunts for a circle and the control is never under the hand that is already resting on the iPad. IT IS THE SAME STICK AND NOT A SECOND ONE — same 130px across, same 53px working radius, and it writes the same `stickVec`, so `step()` never learns any of this happened and nothing downstream had to change. WHAT TELLS A STEER FROM A TAP is the tap/drag split FM4 already uses and not a new rule: under 10px the finger meant to TAP, and tap-to-go handles it; over 10px it meant to STEER, and the stick appears where the finger started. A thumb that is already busy never becomes a stick: not while she is holding the watering can over a crop, not while she is carrying a decoration to put down, not on the corner pad, not with the seed pop-up open. THE CORNER PAD STAYS, FAINT, because a child who learned it should not find it gone; it is the resting one now, same place, same size, still fully live, just washed back so it no longer reads as the way in, and it can go in a session or two. The canvas got `touch-action:none` so a thumb dragged across the world steers and never scrolls the page out from under her. ON THE OTHER HALF, TAP TO GO, THE CARD'S WORDING WAS OUT OF DATE: FM4 and FM6 had already built it, so the work was the three holes they left, and all three are the same hole — things she can tap that she could not actually reach. THE SEED POP-UP OPENED FROM ACROSS THE FIELD: tap a hole thirty units away and the card came up on the spot, so she chose a seed while standing nowhere near where it went in. A walk can now carry a JOB to do on arrival, so the tap starts the walk and the pop-up waits for her feet; standing on the patch already, it still opens at once; and the job dies with the walk, so grabbing the stick mid-journey cancels the planting too, which is the only sane rule, because she changed her mind. A MILL IS NOT A HEN-SIZED TARGET: every tap target had the same 2.1-unit reach and the mill is 6.2 units across, so a tap on the side of the building missed it entirely and she walked to the grass beside it; reach now scales with the thing, and she stops outside the pad it stands on rather than grinding into its wall, still inside feeding range so the feed fires on arrival exactly as walking into it does. THE WELL AND THE BARN WERE NOT TAPPABLE AT ALL: both are solid, so a tap on either aimed her at a point she can never stand on and she stopped dead against it; they now aim her at the spot the thing actually happens from, outside the well and in front of the barn doors, and both reaches are drawn TIGHT, around the building itself and no further, because the well sits on the path between the barn and the field and a generous circle round it would swallow taps that plainly meant "walk over there" — the first cut of this card did exactly that and the FM4 walk-to-open-ground check caught it. NOTHING ABOUT HARVESTING OR FEEDING GOT A SECOND COPY: arriving by tap fires the same proximity checks in `step()` that arriving on foot already fired, and the QA proves each fires exactly once by tapping from ten units away, watching the stack, and then standing there to confirm it does not fire again. THE THUMB HALF IS TESTED WITH A REAL FINGER — `page.mouse` on a phone-sized Chromium, not a hook that sets `stickVec` — because the whole point of the card is where a finger lands on glass, and a hook would prove nothing about that; two new pictures go with it, the floating circle under a thumb in the top-right with the faded corner pad behind it, and one tap with a ring on the ground and no seed card open while she is still walking. The CSS comment claiming drag-anywhere on the world "also works" is gone: it was never built, and now that it is, the comment says what is actually there.

## The Farm says what it means: ask and give are two different pictures (September 8 2026)
**Phase FM, card FM10 — a clarity pass off Mike's playtest, not a feature.** Two things read wrong on a phone. FIRST, A HUNGRY ANIMAL DID NOT LOOK LIKE IT NEEDED ANYTHING: the animal itself never changed, so she walked straight past it. Now the one she is NEAREST to stops what it is doing, turns to face her, droops and sways on the spot, and stands in a soft pulsing ring, while its bubble bobs deeper and swells instead of sitting still. ONE VOICE AT A TIME is the rule: only the nearest hungry animal is loud, because six animals pulsing at once is noise and not an instruction, and further off than 7.5 units nothing changes at all, since an animal frozen and staring at nothing across the island reads as broken rather than as hungry. A MILL DOES NOT DROOP — FM7 already marks the mill and the dairy `machine:true` and that flag is what separates a thing that can lean from a thing that cannot, so the buildings get the ring and the bubble and nothing else. SECOND, "BRING ME THIS" AND "TAKE ME" WERE THE SAME PICTURE: both were the same little model turning in the air, so the two most important sentences in the farm were indistinguishable. They are now two families and the split is a rule, not a one-off. ASK (a hungry animal, the mill, the dairy, the crate) puts the item inside a THOUGHT-BUBBLE OUTLINE, washed cool and soft, hovering with a wobble; an outline and never a filled cloud, or the bubble would hide the very thing it is asking for. Mike picked the bubble over an empty plate and over a question mark, because a question mark is a reading symbol and she cannot read. GIVE (a ready crop, a ready egg, milk, honey, bread, cheese) is warm and bright, BOUNCES UP off the ground at about twice the old hop, and wears the sparkle ring plus a little arrow that only ever points one way: up, onto her stack. One `beatArrow()` drives every arrow on the farm, so a ready crop and a ready egg say it in the same voice rather than in two similar ones. FM7's WANTING-TWO IS A PICTURE NOW TOO: a mill that wants two wheat holds two wheat in its bubble rather than only carrying a 2 on the card. The crate was deliberately kept in the ask family, one step past the card's wording, because it is asking and a bare turning model over it read as something to walk over and take. THE GATE FOR THIS CARD IS A PICTURE and it had to be: a data check cannot tell that two things LOOK alike, which is exactly how they came to be identical, so `qa-farm-shot.mjs` takes a hungry animal and a ready crop in one phone-sized frame from the real game camera, the same pair at desktop width, and the mill holding its two wheat, asserting the frame really holds both families before the shutter goes. IT EARNED ITS KEEP TWICE IN ONE SESSION: the first bubble was four overlapping rings with a two-ring tail, which at this camera distance is scribble and swallowed the item it was framing, and the first arrow was a quarter of its final size in the same pale yellow as the sparkle, so it was invisible on light green grass and the only way to prove it was in the scene at all was to turn it magenta and shoot again. Both faults were DATA-GREEN throughout: `signs()` reported an arrow on all nine ready crops in a frame where not one could be seen. Framing is part of the work too, since the camera looks north and an animal behind her is an animal out of shot: the hens are fifteen units east of the field and can never share a 390-pixel frame with it, so the gate buys the pig and stands her five and a half units south of it, looking up the farm at the field. ONE THING FOUND ON THE WAY: the egg-pickup check has been going red on and off since FM6 and the farm was never at fault — the harness waited on one hen and then walked to `produceSpots()[0]`, which is the first egg on the FARM and not that hen's, so with several animals ready and another egg already drifting in on the magnet it walked to the wrong egg and waited on a stack height something else had satisfied; `produceSpots()` now carries the animal's index, the check walks to the hen it waited on and waits on that hen letting go, and the against-the-wall fallback searches sixteen ways round at three distances instead of six at one. `qa-skyflyer.mjs` 840 green (+13 static), `qa-farm.mjs` 280 green with a new FM10 block, `qa-farm-shot.mjs` three new pictures, `node qa-all.mjs` green. Cache-bust `?v=fm8` -> `?v=fm10`.

## The Farm gets helpers, a cast, a sticker book and a real girl (September 8 2026)
**Phase FM, cards FM8 and FM9.** FM7 gave her things to do; FM8 gives her people to do them with and FM9 gives her a face. THE ONE RULE EVERY HELPER OBEYS: they never take the fun part — harvesting, feeding and the crate whoosh stay hers — and every one of them is slower than she is (she runs 6.4, the dog 4.6, the farmhand 3.9, the tractor 4.4). THE FARMHAND (860) waters and does nothing else: he goes for the crop with the LONGEST left to wait, cannot see a ready one at all, tips his can for a second and a half and takes a third off the wait; the check that proves he never steals her fun watches her tower stay the same height while he works. THE TRACTOR (700) is the last of FM5's hollow boxes and is now the biggest thing on the farm: walk up to it, it asks the ONE question the farm already asks (the seed card, titled "Plant the whole field"), then it drives the rows dropping a seed in every empty patch, buying each seed as it goes and PARKING the moment she cannot afford the next. Her tap and its run now share one door, `plantPatch()`. THE DELIVERY TRUCK comes down the FM6 road on its own as a second, bigger customer: it only buys what the farm MAKES (bread, cheese, honey), pays FIVE times against the crate's three, floats what it wants the way the crate does, unloads on exactly the crate's rhythm, and drives off after a minute and a half whether or not it got anything, so it can never be missed or failed. SIX CUSTOMERS NOW (an owl, a frog and a hedgehog join the bear, fox and bunny) and each has ONE favourite: the slot for it wears a heart, it pays double, and the face jumps when it lands. No words anywhere in that. THE STICKER BOOK stands on a post by the barn door: one stamp per item carried, per animal living here and per helper who turned up, twenty-three today, and the ones she has not got are GREY — the one place in this farm where grey belongs, because here it can only mean "not yet". SHOW A GROWN-UP: one tap takes the whole island from above through the FM6 wide camera and puts it in My Stuff next to her drawings, through the gallery that already exists (`/api/save-art`) rather than a new table, plus an automatic one at each size of farm, never taken mid-whoosh. FM9 — A REAL MAIN CHARACTER: the kid was tube limbs and beads for eyes since FM1 and read as a blue blob; she is now Kenney's characterSmall (CC0) with idle, walk, run and a bend-down pick-up, at exactly the 2.5 units the old kid stood at so the stack still lands on her head and nothing else moved, with a soft contact shadow so she does not float. WHAT MAKES HER A PERSON IS ONE PNG — outfit, hair, face and skin in a single 1024 skin, wired as a NAMED SLOT and never a path, so a different outfit or a character per kid is a one-file change. Mike's call was "the kid picks", so the farm asks once with two pictures and remembers. THE GIRL SKIN THAT ARRIVED IS NOT THE ONE THE FARM LOADS: `skin-farm-girl.png` paints her head a deep maroon while her own arms stay peach, so she read as a dark red mask on a peach body; the farm loads `skin-farm-girl-v2.png`, built from the farm outfit's own skin with her hair composited over it, and the original is untouched and one path away. Every data check passed with the maroon face and only the screenshot showed it, which is the whole reason the picture gate exists. THE CODE-BUILT KID IS STILL THERE, hidden in the same group: abort the model request and the farm opens exactly as it always did, and the harness proves it by aborting the request rather than reading the source. THREE THINGS THIS SESSION FOUND: `window.FARM.coins()` has been returning a DOM node since FM1 (no local `coins` variable, so it resolved to the `<span id="coins">` the browser puts on `window`); a second `customers:` key was silently overriding the new cast reader in the same object literal; and the harness runs at THREE frames a second on the software rasteriser with dt capped at 0.05, so game time there is about a sixth of real time — which is the whole explanation for a set of "flaky" checks, now fixed with a `tractorTick()` lever and waits on state. Character files, licence and loading notes at `public/models/skyflyer/character/` (408KB), registered in the shared asset library as eight `community_sprites` rows via `db/seed-farm-character.sql`, applied and verified in-session. `qa-skyflyer.mjs` 827 green, `qa-farm.mjs` 266 green with new FM8 and FM9 blocks, `qa-farm-shot.mjs` takes nine new pictures, `node qa-all.mjs` green. Cache-bust `?v=fm7` -> `?v=fm8`. FOUR CHECKS WERE ALREADY RED ON `main` before this session (both egg pickups, the first present, the reveal) and a clean checkout of main fails them the same way; every one was the harness measuring itself rather than the farm, and all four are fixed here.

## The Farm gets more to do (September 8 2026)
**Phase FM, card FM7 — more to do.** FM6 gave her an island; FM7 gives her reasons to walk around it. SEVEN SEEDS instead of three (strawberry, tomato and melon join corn, carrot, wheat and the pumpkin; slowest is the melon at fifty seconds, so nothing takes a minute, and a seed still costs what its crop is worth). THE MILL AND THE DAIRY fill the two pads FM6 reserved: two wheat make a loaf, two milk make cheese, and they are fed exactly the way a hen is fed, by walking past holding the thing. They WANT TWO, which is the only new rule in FM7 and it is taught with a two on the want card and nothing else. Bread and cheese are worth more than what went into them, so a trip through a building always pays. THE PIG eats carrots and gives truffles and is the third real library model (`farm-animals.glb` re-cut to Chicken + PekinDuck + Pig, still 126KB). THE HIVE IS NEVER HUNGRY — the bees see to themselves — and the honey comes quicker the more flower beds are standing near it, which nothing on screen says. THE WATERING CAN is fetched from the well once and is hers for good; tipping it over a growing crop takes a slice off what is left, and it cannot be used on an empty or a finished patch. EVERY ANIMAL GETS A NAME AND THE NAME IS A PICTURE: eight shapes, no text box anywhere in this farm and there never will be, and the badge only floats over an animal while she is near it. MAKE IT YOURS: five things to buy (flowers 30, hay 40, fence 25, tree 80, scarecrow 120), placed with two taps and no dragging, and never droppable in the field, a pen, the road or the pond. THE LADDER IS NO LONGER FULL OF EMPTY BOXES: FM5 shipped eight presents with six of them hollow, FM6 filled three and FM7 fills four more, so only the tractor still hops coins back out. THE PICTURE GATE EARNED ITS KEEP: the "Pick a name" card was rendering EMPTY and "Make it yours" was a button that did nothing, because both were wired to variables that had not been looked up yet — every data-level check passed the whole time and only the screenshot showed it. Also found: `PRODUCE_RECIPES.truffle` written before `PRODUCE_RECIPES` existed, and the melon's stripe colour tripping the "nothing costs real money" guard. AND A QA BUG WORTH KNOWING ABOUT: `page.waitForFunction(fn, {timeout: N})` passes the options as the page function's ARGUMENT and silently uses Playwright's thirty-second default — every wait in `qa-farm.mjs` had been doing that for months, which is the entire explanation for the dog and the mill "being flaky" when both worked perfectly. All forty-two are fixed and the file carries a comment saying why. `qa-farm.mjs` 237 green, `qa-skyflyer.mjs` 800 green, `node qa-all.mjs` green, seven new FM7 pictures. Cache-bust `?v=fm6b` -> `?v=fm7`.

## The Farm becomes an island (September 8 2026)
**Phase FM, card FM6 — the land.** The farm stops being a flat green plane with things standing on it. Grass out to a radius of 46, a sand rim, an edge dropping to shallows and open sea, a sky dome with a warm horizon under a blue top, and haze tuned to the same colour so the far shore fades into it. THE WIND IS A SHADER, not a loop: one uniform a frame moves every blade of grass, every leaf and every flower for free, and the grass is 480 tufts in ONE InstancedMesh. Everything is placed from one `ISLE` table — barn centre-north with THE SHOP INSIDE IT (walk to the doors and it opens, walk away and it closes itself), pads reserved for FM7's mill and dairy, field directly south, well between them, coop east and cow pen north-east (both keeping their FM4 gates), pond and bridge south-east, orchard and flower beds and a beehive spot north-west, runway and crate west, a dirt road along the south edge for FM8's truck, stepping-stone paths, hay and logs by the barn, boulders at the edge, pines along the back. NOTHING NEEDED MIGRATING: the save stores patch index and kind order, never a position. THE FIELD GROWS 3x3 TO 5x5 and the new land arrives UNDER PEBBLES AND STUMPS she clears by walking at them (clearing pays her three coins — a call made for Mike; being unable to afford your own field is a strange thing to teach a four-year-old). All 25 slots exist from the first frame but the middle nine are built FIRST, so patch index 0-8 never move and an old save loads with no migration. FOUR REAL BUGS found on the way: the kit rendered BLACK (GLTFLoader marks base-colour textures sRGB and this renderer outputs linear — the kit's textures were moved into the farm's colour space rather than changing every colour Mike has approved); a not-yet-field slot counted as somewhere to plant, so the never-idle count could never hit zero; the barn's auto-close was closing a shop opened with the button from anywhere on the island; and the pathfinder walked her into the field's fence on the way to the animals, so there is now a way-round waypoint. QUATERNIUS: quaternius.com is BLOCKED from this sandbox (proxy 403), so FM6 uses the fourteen pieces already in `public/models/nature` plus everything else built in code; the kit pieces still wanted are listed in `SESSION-LOG.md`. Scene counts were cut after the harness showed a few frames a second on the software rasteriser. `qa-farm.mjs` 200 green, `qa-skyflyer.mjs` 776 green (+18 FM6 static), `node qa-all.mjs` green, island shot at phone/tablet/desktop. Cache-bust `?v=fm6` -> `?v=fm6b`. Also: RLS POLICIES on `farm_saves` applied and verified (a signed-in kid reads and writes only their own row; anon nothing; the API keeps full access).
