...
## Layer three: the studio writes a game nobody has built (September 15 2026)
**Phase CB, card CB5.** Layer one builds a kid's game out of a game we already ship and layer two changes it by asking; LAYER THREE IS THE PROMISE UNDERNEATH BOTH — when a child asks for a lemonade stand or a fishing game and no engine covers it, the studio writes a brand new cartridge instead of handing them the nearest thing with a new title. THE MODEL IS HANDED THIS REPO'S OWN DOCUMENTATION — BUILDING-A-GAME, CARTRIDGE-CONTRACT, GAME-FEEL, HUD-AND-NAV-RULES and MECHANICS, in full, the way a framework hands a model its rules — plus the child's sentence, and must answer with three things: one HTML file, its manifest, and its own cobuild sheet. Those docs live at the repo root and a Vercel function only ships the files its code traces, so `vercel.json` names them in `includeFiles` for this endpoint, and THE FORGE REFUSES TO WRITE ANYTHING IF THE CARTRIDGE CONTRACT COULD NOT BE READ, because a game written without the rules in front of the model is the exact thing this card exists to prevent and that failure would otherwise be silent. NOTHING A MODEL WROTE REACHES A CHILD UNTIL IT PASSES FOUR GATES, cheapest first: STATIC (only our eight shared libraries, nothing loaded from outside, no fetch, no storage, no eval, no iframe, no emoji, a 120KB cap), STRICT (the manifest validated against the sheet the model wrote for its own game, the same strict validation every kid game goes through), THE ROBOT ACTUALLY PLAYING IT level by level, and DID IT TALK TO THE SHELL. The robot is not a second robot: `qa/kid-game-robot.mjs` grew an option to be handed a cartridge that is on no disk, and everything after that is the path Breaker takes — the manifest becomes GAME_CONFIG, the page's own scripts run in the same sandbox, the game is played through the same `sim()` hook — which is why a forged cartridge MUST publish that hook and why the cartridge contract now says so. The sandbox also RECORDS EVERY MESSAGE the game posts while it plays, so "it posts the contract messages" is proven at runtime rather than grepped for: a game that never says it was won is a game no family could keep, however well it plays. THREE ATTEMPTS, EACH ONE TOLD EXACTLY WHAT THE LAST DID WRONG, because a refusal a model cannot read is a refusal that never gets fixed; after that the child gets a game that works, built the ordinary way, and the attempt lands in the review queue. WHAT A CHILD SEES while this happens is not a spinner: Buddy says plainly that nobody has made this one before and that it takes a few minutes, and offers the nearest game we DO have to play meanwhile; the forged game then opens labelled "Buddy is still learning this one", drawn by the shell so a cartridge cannot pretend it is further along than it is; Keep still waits for the family to beat it; and layer three counts as two games on the month's meter. THE CARTRIDGE IS A DATABASE ROW, NOT A BUCKET AND NOT THE REPO — the card asked for Supabase storage, but a table is what this repo already reaches for (image_cache holds every picture the studio paints), it needs no bucket provisioning and no object ACL to get wrong, and it means the file is served ONLY through `/api/forge` where we set the headers: `default-src 'none'`, `connect-src 'none'` so the network is dead at the browser level rather than merely refused by a static check, and `sandbox="allow-scripts"` on the shell's iframe as well. PROMOTION STAGES FILES, IT DOES NOT WRITE TO MAIN: a serverless function cannot write into the repo and more to the point should not, so `/studio/forge-review` (grown-up gated) lists the games that passed with their plays, finishes and hearts, Promote marks the row, and `node scripts/promote-forge.mjs <id>` writes the page, its manifest, its sheet and a generated `qa-<engine>.mjs` that plays it on every QA run, plus the vercel routes, for a person to read like any other change; adding the GAME_CATALOG tile is left to Mike on purpose, and the script prints the line with the family's name already in it. THE SAME QUEUE IS THE ROADMAP: every idea the forge could not finish is listed in the child's own words, which is the most honest product research in this company. ONE THING FOUND ON THE WAY: a forged manifest fell through to the BREAKER level profile and was told it needed bricks and a breaker layout, so layer three could not have existed until the shared loader had a profile for a game it has never seen — `buildable-manifest.js` now has a deliberately loose `forge` profile, and what actually fences a forged game is the cobuild sheet it had to write for itself. `qa-forge.mjs` is 80 checks with no keys and no database: a stub model returns a real working cartridge, all three of the card's sentences forge end to end with every level beaten and the win captured, and then eleven ways of being wrong are each refused one at a time. ONE SCOPE LINE DRAWN ON PURPOSE: a forged game lives in the studio and at its own `/forge/<id>` link and does NOT appear on the shell's My Games shelf, because that shelf is `kid_games` and every row there is gated by CB1's engine check against a cobuild sheet on disk; teaching that gate to accept an engine that exists only as a database row is a real change to the CB1 contract and belongs in its own card rather than being smuggled into this one. The forge needs `ANTHROPIC_API_KEY` to write anything; with no key the studio falls back to layer one and says so, which is a supported state and is tested.
