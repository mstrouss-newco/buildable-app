...
A kids' game builder where children enter their name & age, generate an AI character and world, then play a custom Phaser platformer â all in the browser. No login required.
...
**Phaser 3.60** rendered inside a sandboxed iframe (via a Blob URL) â it is generated
...
some source files used for manual uploads â treat `src/` and `api/` as canonical; do
...
for a production bundle. There is **no separate test suite** â QA is the harness at
...
**Backend & services.** Database is **Supabase (Postgres + RLS)** â tables include
...
**Non-negotiable rules.** This is a **kids' product** â keep everything
...
instead of broken/partial HTML or an HTTP 500 â preserve this guard. Games load sprites
...
currently 13000 for Tier 2 â re-check it if the org tier or rate limits change. (2) The
Phaser canvas has historically rendered **blank inside the iframe**; it is served via a
Blob URL for that reason â test actual rendering, not just a 200 response. (3) Claude
calls are slow (60-90s) and CDP/eval calls time out (~45s), so never await a generation
inline â poll a `window` global. (4) Watch for the duplicate file copies noted above.

**Source of truth for recent changes.** This README contains a dated session log (the
`## ... (June ... 2026)` sections and the `### Commit History`) â that log is the
...
## Next session â start here (owner's queued work)
...
and never handle API keys / billing â surface those to the owner.**

1. **Path A â full multi-genre generator (the big one).** Today the engine only does
   `platformer` and `breakout` (see "Game Types" section below for the Path A/B plan).
   Goal: a real multi-genre generator â next target genre is **Tetris**, then generalize
...
2. **Use the uploaded character sprites for the hero.** Right now the hero is drawn with
   Phaser graphics primitives, NOT loaded from a sprite image â so the DALL-E character
...
3. **Generator tweak: clean sprite URLs, not base64.** See "base64 vs clean URL"
   note below â prefer referencing the GitHub raw library URLs over inlining base64 so
...
4. **Now that we're on Tier 2, consider richer games.** `CLAUDE_MAX_TOKENS` is 13000
   ...
   more detail) â but re-probe for truncation/429 after any bump, and keep the
...
5. **Housekeeping: clean up QA test rows in Supabase.** A few test games/rows were
   ...
   destructive â get the owner to confirm exactly which rows before removing anything.
...
| Image generation | OpenAI `gpt-image-1` (with fallback to `dall-e-3` â `dall-e-2`) |
...
- `asset_id` â unique identifier
- `layer_type` â e.g. sky, midground, foreground
- `category` â theme category
- `image_url` â URL or base64 PNG
- `parallax_speed` â float, used by Phaser parallax scroll
- `theme_tags` â text array for matching
- `prompt_used` â the prompt that generated this image
- `has_transparency` â boolean
- `reusable` â default true
- `created_by_device_id` â anonymous device ID
- `moderation_status` â default 'approved'
...
buildable-app/
âââ api/
â   âââ generate-creature.js   # Character image generation
â   âââ generate-level.js      # World layer image generation
â   âââ generate-game.js       # Phaser game code generation (Claude)
âââ src/
â   âââ BuildableKids.jsx      # Main app orchestrator / state machine
â   âââ CreatorScreen.jsx      # Character + world creation UI
â   âââ LoadingGames.jsx       # Mini-game overlay shown during generation
â   âââ MyStuff.jsx            # Saved games screen
âââ public/                    # Static assets
âââ vercel.json                # Vercel config (maxDuration, rewrites)
...
`vercel.json` sets `maxDuration: 300` for all API functions. Image generation via `gpt-image-1` can take 60â90 seconds per call, so the default 60s timeout is not enough.

---

## Game Mechanics Library (gameplay quality)

Early generated games were thin because the generator prompt only asked for a bare "run and jump over gaps" game â no enemies, power-ups, win condition, or difficulty design. To fix this, gameplay is now driven by a reusable **mechanics library** extracted from a finished, shipped game (*Riley's Garden*, croctot.com/riley).
...
## QA Agent â automated âdoes every level end?â checks
...
**Reference implementation:** the sibling Riley game ships `riley/qa-harness.html` (in the `croc-tot` repo), a single static page that loads the game in an iframe, fast-forwards the loop across all levels with a synthetic âperfect player,â and asserts per-level invariants:

1. **boots** â game globals are reachable.
2. **inBounds** â no enemy sits far outside the playfield.
3. **killGoal** â the kill counter reaches its goal (for `killThenBoss`, 15).
4. **bossSpawns** â a miniboss/boss actually spawns.
5. **ends** â the level reaches the level-complete state within a budget.
6. **noOverrun** â the level never blows past its hard cap.

**Why it pays off:** when the `killThenBoss` mechanic was added to Riley, the harness immediately caught a regression (`ReferenceError: lv is not defined` â the win-check referenced a per-build local instead of the global `LEVELS[idx]`) that made every level unwinnable, before any player saw it.

**For Buildable Kids:** the same harness can be pointed at any generated game by setting the iframe `src` to that gameâs Blob/preview URL. The roadmap is to run these invariants automatically after generation (and/or in a Vercel function) and flag any game where a level fails to reach completion, so âunwinnable levelâ bugs are caught at build time rather than by kids. The invariants mirror the `killThenBoss` primitive in `MECHANICS.md` â generated games that use it should pass by construction.

---

## QA Session Log â June 7 2026
...
**Fix:** Added multi-model fallback: try `gpt-image-1` â `dall-e-3` â `dall-e-2` in sequence. First successful response is used.
**Commits:** `403cc8f`, `a2ef6c9`

#### 3. Vercel 60s function timeout too short
**File:** `vercel.json`
**Error:** 504 Gateway Timeout on `/api/generate-level`
**Fix:** Raised `maxDuration` from 60 â 300 seconds.
**Commit:** `6044c16`
...
2. Character generated â AI art via `gpt-image-1`, saved to `community_characters`
3. World generated â 4 parallax layers saved to `community_layers`
...
`published_games` (and the asset INSERTs) were created/run with the Supabase linter's **"Run without RLS"** option, matching the existing community/published tables which are read via the service key. (This corrects the older note above stating all tables have RLS enabled â the app does **not** rely on RLS for reads.)
...
## Blank Game Canvas Fixed â Blob URL Render (June 7 2026, later session)
...
### Root cause
`PlayGameScreen` (in `src/BuildableKids.jsx`) injected the generated game HTML into the play `<iframe>` using `doc.open()` / `doc.write(gameHtml)` / `doc.close()`. A `document.write()`-populated iframe does not get a proper browsing context, so Phaser 3 (and its WebGL/canvas init) intermittently failed to boot and the canvas rendered blank. The game code and the Phaser script were present the whole time â the engine just never initialized.
...
- **Keep the injected HTML small.** Bias layer/sprite selection toward the clean `created_by_device_id = 'asset-pack'` rows (GitHub raw URLs) and skip rows whose `image_url` starts with `data:`. Large base64 layers bloat the game HTML and slow the iframe paint â this compounds render problems even with the Blob URL approach. (See the base64-vs-clean-URL note above.)
...
After the Blob URL render fix, end-to-end QA against the live demo showed games **still** rendering a blank canvas â but for a different reason. Inspecting the play iframe directly: the Blob URL loaded fine, Phaser 3.60.0 loaded from CDN, and the ~19KB inline game script was injected â but **no `<canvas>` was ever created**. Evaluating the inline script threw `SyntaxError: missing ) after argument list`. The generated code had unbalanced brackets (e.g. 344 `(` vs 343 `)`, 75 `{` vs 73 `}`) and contained **no `new Phaser.Game(...)` call at all**.

### Root cause
The Claude call in `api/generate-game.js` used `max_tokens: 8000`. A full, polished game (the prompt asks for enemies, power-ups, a difficulty ramp, and a separate CONFIG block) routinely needs more than that, so the response was **truncated mid-script**. The only validation gate was `html.includes("<!DOCTYPE")` â a truncated file still starts with `<!DOCTYPE html>`, so the broken code passed the check and was served as-is. `fallbackGame()` never triggered because its trigger condition did not detect *malformed* output, only a missing/empty response.
...
- **Always check the completion `stop_reason`.** `max_tokens` means the answer was cut off â do not use it. Size `max_tokens` to the realistic worst-case output, not the average.
...
**Problem:** the first real generated game rendered, but sprites were chaotic â mis-scaled,
...
**File:** `qa/game-qa-harness.html` (standalone â open in a browser, no build step)

A self-contained page that loads a generated game in a **sandboxed iframe** and asserts:
- **Layer 0 â API:** `/api/generate-game` returns 200 (never 500), non-empty html, and is
  not a silent fallback (`source !== "fallback"`).
- **Layer 1 â Boots:** has doctype + closing `</html>`, bootstraps Phaser, has balanced
  `{}`/`[]` in its scripts, and uses `setDisplaySize` (coherence signal).
- **Layer 2 â Playable:** a `<canvas>` mounts at non-zero size, the Phaser global is present,
...
1. **Layer 1 â Does it boot?** (automated) doctype, Phaser bootstrap, balanced brackets,
   canvas mounts. Enforced in `validateGameHtml` (server) + the harness (client).
2. **Layer 2 â Is it playable?** (automated) input moves/responds, a win condition and a
   lose condition are both reachable, no soft-lock.
3. **Layer 3 â Is it coherent?** (prompt-enforced) sprites correctly sized/positioned, no
   overlap, HUD readable â see the VISUAL COHERENCE RULES above.
4. **Layer 4 â Is it fun?** (human / kid testing) the only layer a machine cannot judge.
...
- **Constrain LLM-generated layout with hard numeric rules** (exact sizes, spawn coords,
  safe zones) rather than adjectives like "nicely arranged" â models comply far better.
...
## Game Types: Breakout added (Path B) + Multi-Genre Roadmap (Path A â planned)
...
produced just by adding art or a mechanic row â the generator prompt has to change.
...
- **Reuses the SAME library sprites as bricks** (coin/gem/star/heart/chest/etc.)
  arranged in a grid â so it ships with **zero new art required**. Missing subjects
...
"breakout"`. (The UI does not yet expose a game-type picker on the create screen â that
...
> Design note: this keeps the "skin vs engine" principle but adds a third axis â **genre**.
...
`db/seed-breakout-mechanic.sql` â idempotent upsert that adds
...
### Sprite coverage audit â RESULT: 100% complete, ZERO gaps
...
**No new sprite art is needed** â the library fully covers all 7 themes x 9 subjects.
...
### Breakout QA probe â BLOCKED by a production deploy / API-key issue (needs attention)
...
cause is **not** the Breakout code â it is the production deployment:
...
- More importantly, even plain platformer requests now return a **~2.3KB fallback with
  ...
  signature matches the old code path `if (!claudeKey) return fallbackGame(...)` â
  ...
  key was working before â something changed in the env/deploy since.)

**Action needed (owner, in Vercel dashboard â cannot be done from the app):**
...
## Session update â root cause CONFIRMED via live probes (429 â truncation)

After deploying the rate-limit fix (`max_tokens` 16000 â 7000 + retry/backoff on
429/529), live probes against the production `/api/generate-game` were re-run.

**What the probes showed:**
- The **429 rate_limit_error is gone** â the retry/backoff is working and we are no
...
- `max_tokens` high enough for a full game (~12â16k)  ->  hits the 8k/min **429 cap**.
...
(target ~12000â14000) so games generate completely AND stay under the higher cap.
...
## RESOLVED â Tier 2 + max_tokens 13000: full games generating (June 7 2026, later session)
...
**Live production probes after deploy â both PASS:**
...
## Platformer Controls + Win-Condition Fix â frozen hero / 0-star auto-win / no mobile controls (June 8 2026)

End-to-end QA of the live demo (a saved platformer, "Floating Valley Quest") found the generated game was **unplayable**: the hero never moved or jumped, and the game declared **"You Win â All Stars Collected! Score: 0"** when the timer ran out with 0/2 stars. Root cause was traced to the **platformer prompt** in `api/generate-game.js`, not the engine/render path.

### Root causes (all in the platformer prompt's Technical requirements)
- **No horizontal movement.** The `Controls` line only said "SPACE or UP to jump, double-jump allowed. Touch/click also jumps" â it never told Claude to implement LEFT/RIGHT walking. Generated heroes had no `setVelocityX` path, so they were frozen in place (looked like a broken auto-runner).
- **Auto-win on timeout.** The win/lose instruction was loose ("keep a clear win/lose condition and an anti-soft-lock failsafe"), with no rule that winning requires collecting the objectives. The failsafe ended up firing a WIN when the countdown hit 0 â even with score 0 / 0 stars.
...
### Verification
Rendered the new fallback in an iframe and inspected the live Phaser scene: `lives === 3`, world width 3200, 10 coins / 5 spikes / goal present, 3 hearts, `pixelArt` + `roundPixels` true, `scaleMode` FIT. Drove the hero right and confirmed it travels through the world â **hero x went 100 -> 1647 and camera scrollX 0 -> 1239** â and that running into 3 spikes greyed out all 3 hearts before Game Over (lives system working). Generated game HTML parses and brackets balance within `<script>` (passes `validateGameHtml`).
...
- **"My Stuff" (client-side, IndexedDB via `src/store.js`)** saves only **characters and
  levels (skins)** â there is **no `html` field**. Tapping **Use â¶** in `MyStuff.jsx` feeds
...
- **My Stuff** (in the live app): create/save a character + level, then **Use â¶** to play a
...
Added **`db/align-platformer-mechanic-lives.sql`** (commit `4b62f2d`) â idempotent,
...
DB hint at build time, so this is consistency-only â newly generated games are already 3-life.)
...
(~110-115s) â poll a window global, do not await inline (known gotcha).
...
### Owner to-do (needs the service key / a destructive op â cannot be done from automation)
...
2. Optionally delete the QA test row `published_games` `game_id = qaa95cb6` (and
   `community_levels` id 7) â destructive, so left for the owner.
...
- **`api/admin-stats.js`** â aggregates live counts (community_characters / community_levels
  / saved_games / published_games / game_mechanics) and cost. Cost comes from the
  `usage_log` table when present; otherwise falls back to an estimate (counts Ã unit cost).
  Reads DAILY_BUDGET_USD for budget %.
- **`api/admin-list-games.js`** â returns ALL games (saved_games + published_games) with
...
- **`AdminDashboard.jsx`** rewritten: Overview reads /api/admin-stats (real counts + cost,
  estimate flagged); new **ð® Games tab** lists every game with a Play/QA link and an
...
**Live probe after deploy:** /api/admin-stats returned real data â counts (13 levels, 1
...
**`db/create-usage-log.sql`** â idempotent `create table if not exists usage_log`
...
populated in prod â cost source came back as usage_log.) Generation endpoints can insert one
...
- **`api/list-assets.js`** â returns the real library art (community_layers + community_sprites)
...
- **`CreatorScreen.jsx`** rewritten with visual pickers:
  - **Character screen:** "Make your hero!" â tappable emoji trait chips (fluffy/sparkly/fire/â¦)
  ...
  - **Level screen:** "Build your world!" â each theme is a card showing that theme's real
...
### Commits
`95cc6ad` admin-stats Â· `db3e2c9` admin-list-games Â· `487e676` usage_log migration Â·
`88b6b73` AdminDashboard live wiring + Games tab Â· `76d8b9e` list-assets Â·
...
- `saved_games` and `community_characters` counts came back 0 â confirm those tables exist /
...
### 2026-06-08 â Usage logging wired into all generators (admin cost/volume)
...
- `api/generate-game.js`: added a best-effort `logUsage()` helper (matches
  ...
  row (`kind:"game"`, cost $0 â library assembly is free â with meta: gameType,
...
HTML, mechanic + 9 sprites (build ~125s) â so the success path incl. logUsage ran.
...
## Admin Dashboard 401 fixed â session-token auth (no raw secret in the browser) (June 9 2026)

Owner report this session: "admin isn't working ... we just set this up ... we may need more pipes to connect." Reproduced and fixed.

### Symptom

Opening the Admin Dashboard on the live demo (buildablekids.com/demo -> ð Admin -> Overview) showed **"Couldn't load stats: request failed: 401"**; the Games / Characters / Levels tabs failed the same way. Direct probes confirmed both `/api/admin-stats` and `/api/admin-list-games` returned HTTP 401 `{"error":"unauthorized"}` â even when sending an `x-admin-token` header.

### Root cause

This is the second half of the previously-open owner to-do ("Set ADMIN_API_TOKEN in Vercel + paste it into the admin Settings tab"). The env var `ADMIN_API_TOKEN` **is now set on the server**, so the admin endpoints correctly require a matching `x-admin-token` header (`/api/admin-session` reports `locked: true`). But the dashboard only sent that header if the operator had manually pasted the raw token into Settings -> localStorage(`adminApiToken`), which was empty. So the front end and the now-locked API were disconnected â the missing "pipe." (The Supabase data layer was healthy the whole time.)

### Fix (Option B â session token, chosen over "just paste the secret")
...
- **New `api/_adminAuth.js`** â shared verifier. `isAdminAuthorized(req)` accepts EITHER (a) a signed session token (HMAC-SHA256 of an expiry timestamp using `ADMIN_API_TOKEN` as the signing secret, verified with `timingSafeEqual` + not-expired check), OR (b) the legacy raw `ADMIN_API_TOKEN` as `x-admin-token` (back-compat). If `ADMIN_API_TOKEN` is unset, stays open for local dev. Also exports `mintSessionToken(secret, ttlMs)`.
- **New `api/admin-session.js`** â `POST { password }`. Verifies the password against `ADMIN_PASSWORD` env (falls back to the same default the client uses) and returns a 30-min signed token + `exp` (`locked: true`). On wrong password -> 401. The real secret is used only to sign; it is never returned.
- **`api/admin-stats.js` + `api/admin-list-games.js`** â inline auth block replaced with `if (!isAdminAuthorized(req)) return 401`.
- **`src/AdminDashboard.jsx`** â `handleLogin` is now async: after the local password check it POSTs to `/api/admin-session`, stores the returned **signed** token (not the raw secret) in localStorage(`adminApiToken`), and sends it as `x-admin-token` on admin calls. `handleLogout` clears it. `fetchAdmin` now turns a 401 into a clear, actionable banner ("Admin session expired or not authorized. Please log out and sign in againâ¦") instead of a raw "request failed: 401". The manual token field in Settings is kept as an override/fallback.
...
- No need to paste `ADMIN_API_TOKEN` into Settings anymore â login mints the token automatically. (The Settings field remains as a manual override.)
...
## Owner's queued work â cost-per-type, element inventory, character reuse, library-before-DALL-E (June 9 2026)
...
### 1. "How much does a game / character / level cost to make?" â per-TYPE cost breakdown

Today `usage_log` already stores a per-call `kind` ('character' | 'level' | 'game' | 'quiz') and `cost_usd`, but `/api/admin-stats` only sums them into one today/month total â there is no per-type view. PLAN: aggregate `usage_log` by `kind` in admin-stats (count + total + avg cost per kind) and show a "Cost per type" table on the Admin Overview, so the owner can see e.g. avg $/character vs $/game. (Library-driven games/levels log $0; image generations are the real cost.)

### 2. "How many level elements do we have?" â element inventory
...
- Client: `store.js` `saveCharacter()` has NO dedup â no find/findIndex/name match. It just prepends a new item with a fresh id on every call, and `BuildableKids.jsx` auto-saves a character after each creation without first checking `listCharacters()` for an existing match. So each "kid with jetpack" creation = a brand-new saved entry.
- Server: `api/generate-creature.js` never SELECTs `community_characters` for an existing match before generating â it always calls OpenAI (2-model fallback) and INSERTs a new row. So the same prompt regenerates (and re-pays for) fresh art every time.
...
## DONE â cost-per-type, element inventory, character dedup, library-before-DALL-E (June 9 2026, later session)
...
### 1. Cost per type â DONE
`/api/admin-stats` now aggregates `usage_log` by `kind` into a `perType` array (count, total, average $). The Admin Overview shows a new "ð¸ Cost per type" table. Live verify: image generations = 25 calls, avg ~$0.0592, total $1.48; library games/levels correctly log $0 (so they don't inflate cost). Image generation is confirmed as the only real cost driver so far.

### 2. Level-element inventory â DONE
`/api/admin-stats` now returns an `inventory` object (layers + sprites per theme, clean-URL vs legacy base64). New "ð§± Level elements (library)" card on the Overview. Live ground truth: **layers 28 clean / 47 total** (19 legacy base64), **sprites 63 clean / 63 total**, with a per-theme table. Finding surfaced: 19 base64 layer rows are the bloat source, and `/api/list-assets` was hiding ALL layers from the kid world-builder because they were base64-only.

### 3. Characters not saving ("kid with jetpack" duplicates) â DONE (client) + scoped
Root cause was client-side: `store.js` `saveCharacter()` had no dedup. Fixed: it now keys on normalized (trim+lowercase) name+description; a repeat save UPDATES the existing character in place and moves it to the front (with `updatedAt`) instead of adding a duplicate. Verified the dedup logic is live in the deployed bundle. Note: the kid demo flow does NOT call `/api/generate-creature` at all (it only calls `/api/generate-game`), so there was no DALL-E character regeneration to stop â the duplication was purely the local save. `generate-creature.js` (unused by the demo) was left as-is; if it is ever wired in, add the same reuse-before-DALL-E guard there.

### 4. Use our elements before DALL-E (+ candy theme bug) â DONE
Two fixes across the asset path:
- **Theme normalization (`normTheme`)**: added to `api/list-assets.js`, `api/generate-game.js`, and `api/generate-level.js` so the UI's short label "candy" matches the library's "Candy kingdom" tags (case-insensitive). Before: `/api/list-assets?theme=candy` returned 0 sprites (â all gaps â DALL-E). After (live): candy returns **4 layers + 9 sprites**, and a live candy game generation returns `source: library`, **spriteGaps: [] (zero)**, all 9 sprites used.
...
- Replace the 19 legacy base64 layer rows in `community_layers` with clean GitHub-raw URLs (or delete the base64 dupes) so the world-builder shows background layers to kids and clean-bias has clean layers to pick. This is a DB data change â left for the owner.
- A stray `diagtest` theme shows up in the inventory (leftover QA rows) and the earlier `qaa95cb6` / `community_levels` QA rows are still flagged for optional cleanup (destructive â owner-run).


---

## Session log — 2026-06-09 (cleanup SQL + agent learnings store)
...
- **`db/cleanup-base64-layers.sql`** — removes the 19 legacy base64 rows from
...
- **`db/cleanup-qa-rows.sql`** — removes the stray `diagtest` theme rows and the
...
- **`db/agent-learnings.sql`** — new idempotent `agent_learnings` table
  (source, project, category, title, detail, tags[], meta jsonb, created_at) + indexes.
- **`api/log-learning.js`** — POST endpoint so the external mechanics agent can persist
...
### Owner to-do (you must run these — destructive / needs service key)
1. Run `db/agent-learnings.sql` once in Supabase to create the table (required before
   `/api/log-learning` works).
2. Run `db/cleanup-base64-layers.sql` — review Step 1, then flip Step 2 to `commit;`.
3. Run `db/cleanup-qa-rows.sql` — review Step 1, then flip Step 2 to `commit;`.
