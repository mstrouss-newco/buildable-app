...
// Kids type free text into the `theme` and `prompt` fields, so those are the
// only places unsafe input can enter (the other choices are dropdowns mapped
// to fixed dictionaries below). We screen that free text against a wordlist of
// violence / gore / weapons / adult / drug / profanity / hate terms BEFORE we
// build a brief or spend anything on ElevenLabs. Word boundaries (\\b) keep us
// from false-flagging innocent words that merely contain a short token.
const BLOCKED_PATTERNS = [
  // violence / gore
  /\bkill(s|ing|ed|er|ers)?\b/, /\bmurder(s|ing|ed|er)?\b/, /\bblood(y|ied|ies)?\b/,
  /\bgore\b/, /\bgory\b/, /\bguts?\b/, /\bstab(s|bing|bed)?\b/,
  /\bshoot(s|ing)?\b/, /\bshot\b/, /\bgun(s|shot|shots)?\b/, /\bknife\b/, /\bknives\b/,
  /\bdead\b/, /\bdeath\b/, /\bdie(s)?\b/, /\bdying\b/, /\bkilling\b/,
  /\bslaughter/, /\bbehead/, /\bdecapitat/, /\btortur/, /\bmaim/, /\bmutilat/,
  /\bhang(s|ing|ed)?\b/, /\bstrangle/, /\bchoke(s|d|ing)?\b/, /\bsuffocat/,
  /\bsuicide\b/, /\bself.?harm/, /\bbomb(s|ing)?\b/, /\bexplos/, /\bterror/, /\bweapon/,
  /\bviolen(t|ce)/, /\bgun\b/, /\bcorpse/, /\bzombie.?(kill|blood|gut)/,
  // adult / drugs / alcohol
  /\bsex(y|ual)?\b/, /\bnude\b/, /\bnaked\b/, /\bporn/, /\bboob/, /\bbutt.?(sex)?/,
  /\bdrug(s)?\b/, /\bcocaine\b/, /\bheroin\b/, /\bmeth\b/, /\bweed\b/, /\bmarijuana\b/,
  /\bbeer\b/, /\bvodka\b/, /\bwhiskey\b/, /\bwine\b/, /\bdrunk\b/, /\balcohol/,
  // profanity roots (incl. common masking with * or numbers)
  /\bf+[u\*]+c*k/, /\bsh[i\*1]t\b/, /\bb[i\*1]tch/, /\basshole/, /\bdamn\b/, /\bcrap\b/,
  /\bdick\b/, /\bpiss\b/, /\bslut\b/,
  // hate
  /\bhate\b/, /\bracist\b/, /\bnazi\b/,
];
...
// Returns { field, term } for the first flagged free-text field, or null if clean.
function screenChoices(c) {
  for (const field of ["prompt", "theme"]) {
    const hit = screenText(c[field]);
    if (hit) return { field, term: hit };
  }
  return null;
}
...
export default async function handler(req, res) {
  ...
  // Kid-safety gate: screen free-text fields BEFORE building a brief or
  // spending on ElevenLabs. Blocked requests get a friendly nudge, not a song.
  const blocked = screenChoices(choices);
  if (blocked) {
    return res.status(200).json({
      ...
      field: blocked.field,
      message: "Let's keep songs fun and friendly! Try a different idea \u2014 like animals, space, sports, robots, or a birthday party.",
    });
  }
