...
import { thumbForWorld, songCover } from "./_thumbs.js";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (kind === "song") {
      ...
      items = rows.map((r) => ({ kind: "song", id: r.song_id, title: r.title, creator: r.kid_name || "A kid", cover_color: r.cover_color, vibe: r.vibe, theme: r.theme, audio_url: r.audio_url, thumbnail: songCover(r.vibe, r.theme), play_count: r.play_count || 0, heart_count: r.heart_count || 0, created_at: r.created_at, meta: r.meta || null }));
    } else if (kind === "story") {
      ...
      items = rows.map((r) => ({ kind: "story", id: r.story_id, title: r.title, creator: r.kid_name || "A kid", cover_color: r.cover_color, world: r.world, thumbnail: thumbForWorld(r.world) || null, play_count: r.play_count || 0, heart_count: r.heart_count || 0, created_at: r.created_at }));
    } else {
      ...
      items = rows.map((r) => ({ kind: "game", id: r.game_id, title: r.title, creator: r.creator_name || "A kid", theme: r.theme, mechanic: r.mechanic_name, character: r.character_name, preview_image_url: r.preview_image_url, thumbnail: r.preview_image_url || thumbForWorld(r.theme) || null, play_count: r.play_count || 0, heart_count: r.heart_count || 0, created_at: r.created_at }));
    }
