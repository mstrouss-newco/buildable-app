...
export default async function handler(req, res) {
  ...
  const prompt = [
        ...
        ``,
        `Build a side-scrolling action game (runner where the world scrolls right-to-left, hero on the left). It MUST be genuinely fun, not just run-and-jump. Use the proven mechanics below.`,
        ``,
        `=== MECHANICS LIBRARY (use these — they are tested patterns from a shipped game) ===`,
        `ENEMIES: spawn 2-4 enemy types that the hero must avoid or defeat. Give each a named movement pattern:`,
        `  - linear: straight line with a gentle bob (easy intro enemy)`,
        `  - patrol: moves back and forth, steps toward the player at each turn`,
        `  - zigzag: sine-wave weaving (flying enemy)`,
        `  - swoop: drifts, then periodically DIVE-BOMBS toward the player and climbs back (aggressive)`,
        `  Implement each pattern in the sprite's update using time/delta. Make enemies match the ${levelTheme} theme.`,
        `AUTO-HELP (kid-friendly, ages 5-12): give the hero an auto-firing or auto-aim helper so young kids don't have to aim. The helper targets the nearest enemy automatically.`,
        `COLLECTIBLES & POWER-UPS: scatter collectible items (theme-appropriate). Plain ones add score; some are POWER-UPS that charge a meter and temporarily upgrade the hero's attack when full. This creates risk/reward, not just coin-grabbing.`,
        `DIFFICULTY CURVE: start easy and ramp up — increase enemy count and speed over time, and introduce harder patterns (linear -> patrol -> zigzag -> swoop) as the score climbs.`,
        `WIN/LOSE: clear lose condition (hero hit too many times / falls) with a friendly Game Over + Play Again. NEVER allow a soft-lock: if a goal could become unreachable, include a failsafe timer that resolves it. Keep game-over gentle for young kids.`,
        `STRUCTURE (so games are remixable later): put all skinnable values — hero look, theme colors, item list, enemy definitions — in a clearly-marked CONFIG object near the top of the script, separate from the engine/game-loop code.`,
        `=== END MECHANICS LIBRARY ===`,
        ``,
        `Technical requirements:`,
        `1. Load Phaser 3 from CDN: https://cdn.jsdelivr.net/npm/phaser@3.60.0/dist/phaser.min.js`,
        `2. Draw the hero and enemies with Phaser graphics or emoji text — NO external image files.`,
        `3. Use colors matching the ${levelTheme} theme for background, platforms, and enemies.`,
        `4. Controls: SPACE or UP to jump, double-jump allowed. Touch/click also jumps (mobile).`,
        `5. Show score (distance + points) top-left in a friendly font, and show ${charName} as a label above the hero.`,
        `6. Canvas 800x400, centered, dark body background. Auto-start, no splash screen.`,
        `7. Colorful, readable, and fun for ages 5-12.`,
