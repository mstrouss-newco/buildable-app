...
const SCREEN_WORD = "word_buddies";
...
function GamePicker({ onHome, onPlatformer, onSurvival, onBreaker, onRunner, onChess, onTyping, onTennis, onWordBuddies, onSounds }) {
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={{ width: "100%", maxWidth: "620px", marginTop: 20, display: "grid", gap: 18, gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))" }}>
        ...
        {tile("linear-gradient(160deg,#F6B8D0,#A78BFF)", "Word Buddies", "Make little words — your Helper spells them out loud!", onWordBuddies, false, "word-buddies")}
...
function WordBuddiesScreen({ onHome }) {
  return (
    <div style={{ position: "fixed", inset: 0, background: "#FBF3E6", zIndex: 50 }}>
      <button onClick={onHome} style={{ position: "absolute", top: 14, left: 14, zIndex: 2, fontFamily: NUN, fontWeight: 800, fontSize: 14, color: "#4a3b78", background: "rgba(255,255,255,0.92)", border: "1px solid rgba(124,92,252,0.25)", borderRadius: 999, padding: "8px 16px", cursor: "pointer" }}>Back</button>
      <iframe title="Word Buddies" src="/word-buddies.html?v=1" style={{ width: "100%", height: "100%", border: "none", display: "block" }} />
    </div>
  );
}
...
export default function BuildableKids() {
  ...
  if (screen === SCREEN_WORD) {
    return <WordBuddiesScreen onHome={() => setScreen(SCREEN_GAME_PICKER)} />;
  }
