...
- [x] **Session 6C — First studio converts.** Music Maker gets a studio manifest: badge on the picker, coins, customization (instrument packs), learning gates. Proves type studio.
...
- [ ] **Session 9D — Retire old engine menus.** Blocked until a new kid-facing level maker replaces the old Make a level screen. Then delete the old start menu, worlds tab, losing screen, and leftover menu code. Links already point away from them.
- [ ] **Session 9E — Editor async QA gate.** After an editor save publishes, automatically run the game's QA robot (GitHub Action or scheduled job) and alert on failure with easy revert. Upgrades the editor's structure-check-on-save into the full no-impossible-levels guarantee, mainly for difficulty changes.

## Phase 10 — Easy parent login
- [ ] **Session 10A — Magic link + co-parent QR.** Add "Email me a login link" to the Grown-ups screen using Supabase's built-in email link sign-in (no password ever), redirecting to /app like Google OAuth. Show the existing 6-character family code as a QR in the Grown-ups portal (small client-side QR drawing library, no emojis); a second grown-up scans it, signs in via magic link, and joins the family automatically through the existing join-by-code system. No database changes. Note: Supabase's built-in email sender is rate-limited to a few per hour; upgrade the sender if usage grows.
- [ ] **Session 10B — QR new-device sign-in.** WhatsApp-style handoff: the logged-in device shows a QR in the Grown-ups portal; a new device scans it and is signed in. One new table (device_link_tokens: hashed one-time token, 5-minute expiry, single use) shipped as an idempotent SQL file in db/ for Mike to run, plus one new service-role endpoint (api/device-link.js) that validates the token and redeems a Supabase admin magic link for the parent's email. Reuses the invite-token pattern from api/invite.js and the QR library from 10A. Do 10A first.
