...
(function (g) {
  ...
  // GN4 — the shell needs to know the MOMENT an engine leaves its own picker and
  // starts playing, because that is when the bottom bar has to disappear. Engines
  // were supposed to call BuildableGameNav.update() on every state change and
  // several never did (bubble, runner, tank, mathcannon), so the shell only ever
  // saw the state at registration. Rather than wire a call into every transition
  // in every engine -- twenty places to drift -- the bridge watches the value
  // itself and posts only when it flips. One file, every engine, nothing to
  // forget. Reading a boolean four times a second costs nothing.
  var lastInGame = null;
  function watchInGame() {
    if (!cfg || !iframed()) return;
    var now = cfg.inGame ? !!cfg.inGame() : true;
    if (now === lastInGame) return;
    lastInGame = now;
    postState();
  }
  ...
  BN.register = function (c) {
    ...
    g.addEventListener("message", function (e) {
      ...
      // GN4: the shell says whether its bottom bar is currently over this game.
      else if (d.type === "nav:bar") { setBarStrip(!!d.on, d.h); }
    });
    ...
    lastInGame = cfg.inGame ? !!cfg.inGame() : true;
    setInterval(watchInGame, 250);
  };
  ...
  // --------------------------------------------------------------------------
  //  GN4 — the bottom bar over the game, and the two things that takes.
  //
  //  1. THE RESERVED BOTTOM STRIP. While the shell is showing the five-tab bar
  //     over this iframe it posts nav:bar {on, h}. We publish that as
  //     --bk-nav-bottombar and mark the page .bk-barup, so a picker (the shared
  //     buildable-startscreen.js, or an engine's hand-rolled one) can pad its
  //     last row clear of the bar. It is 0px whenever the bar is down, so
  //     gameplay never loses the space.
  //
  //  2. THE TOUCH CATCHER. Same fault class as the Home-tap fix above: on iOS a
  //     touch on a shell element overlapping an iframe is routed INTO the iframe,
  //     so the bar would look tappable and do nothing on a phone while working
  //     fine with a desktop mouse. So we put an invisible catcher over the bar's
  //     own strip INSIDE the game and forward the tap's viewport coordinates to
  //     the shell, which hit-tests its own DOM and presses the real button.
  //     Forwarding COORDINATES rather than a tab name is deliberate: the shell
  //     owns the bar's layout, and duplicating the five-tab geometry in here
  //     would be a second copy to drift. The catcher exists ONLY while the bar
  //     is up -- which is only while the engine reports it is not playing -- so
  //     it can never swallow a gameplay tap.
  // --------------------------------------------------------------------------
  var BAR_ID = "bkNavBarCatcher";
  function setBarStrip(on, h) {
    try {
      if (!g.document || !g.document.documentElement) return;
      var el = g.document.documentElement;
      el.style.setProperty("--bk-nav-bottombar", (on ? (h || 76) : 0) + "px");
      if (on) el.classList.add("bk-barup"); else el.classList.remove("bk-barup");
      var z = g.document.getElementById(BAR_ID);
      if (!on) { if (z && z.parentNode) z.parentNode.removeChild(z); return; }
      if (!g.document.body) return;
      if (!z) {
        z = g.document.createElement("div");
        z.id = BAR_ID;
        z.setAttribute("aria-hidden", "true");
        var last = 0;
        var fire = function (ev) {
          ...
          try { ev.preventDefault(); ev.stopPropagation(); } catch (e) {}
          var t = Date.now(); if (t - last < 400) return; last = t;   // one tap, not three
          try { g.parent.postMessage({ type: "nav:barTap", x: pt.clientX, y: pt.clientY }, "*"); } catch (e) {}
        };
        var shield = function (ev) { try { ev.stopPropagation(); } catch (e) {} };
        z.addEventListener("touchstart", shield, { passive: true });
        z.addEventListener("pointerdown", shield);
      ...
      z.style.cssText = "position:fixed;left:0;right:0;bottom:0;height:" + (h || 76) +
        "px;z-index:2147483000;background:transparent;-webkit-tap-highlight-color:transparent;touch-action:manipulation;";
    } catch (e) {}
  }
  BN.setBarStrip = setBarStrip;
